...
def efield_vs_angle(h, decay, z, f_Lo_list, bandwidth):
    ...
    i_off_angle_array = list(range(0, len(off_angle_array)))
    ...
    for i_offangle in i_off_angle_array:
        ...
        efield = np.sqrt(pow(efield_td[i_offangle][b'x_v_per_m'],2) + pow(efield_td[i_offangle][b'y_v_per_m'],2))
        time = efield_td[i_offangle][b'time_s']
        time -= time[0]
...
def write_interpolator_file(h, epeak_array, f_Lo_list, psi_list, zenith_list, decay_list ):
        ...
        interp_file = open(os.environ['TAU_ACC_ZHAIRES_DIR']\
                               +'/interpolator_efields_%2.1fkm.npz'%(h), 'wb')
        ...
        for i_f_Lo, f_Lo in enumerate(f_Lo_list):
                ...
                interpolator = scipy.interpolate.LinearNDInterpolator(
                                      list(zip(dd.flatten(), zz.flatten(), pp.flatten())),
                        ...
                        for d in np.arange(0, 10,1):
                                if (d<a)and(a>0.5) or (d<=a)and(a==0.5):        
                                        ...
                                        for ize, ze in enumerate(zenith_list):
                                                ...
                                                print(clean_parms[cut].gauss_blur.values)
