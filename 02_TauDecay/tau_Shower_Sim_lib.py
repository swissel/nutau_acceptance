...
threshold_voltage = 8.e-6 # V, this is based on taking half the peak field for the weakest ANITA-1 EAS candidate. (0.466 mV/m convolved with antenna effective height at 300 MHz). 
...
def E_field_model_ANITA_UHF_87deg(view_angle_deg, log10_tau_energy, distance_km):
    # Lorentzian beam pattern based on Harm's results
    # Returns electric field peak in V/m
    
    # This is a more detailed fit to the lateral distribution function at 87 for a 200 - 1200 MHz band.
    E_0  = 1.22543257e-05    # V/m at ground level for 10^17 eV tau lepton.
    frac_gauss = 1.31588330e-01 # fraction of Gaussian peak ( 1-frac_gauss is lorentzian)
    peak_view_angle = 1.41098108e+00 # degrees
    width = 1.31749036e-01       # degrees
    E_1 =1.85991817e-07    # V/m at ground level for 10^17 eV tau lepton.
    width2 = -8.85539420e-01          # degrees
    v = (view_angle_deg-peak_view_angle)/width 
       # The E_field model is a linear combination of a Gaussian and a Lorentzian centered at the peak view angle. A Gaussian centered at 0 deg is added to account for the central lobe. 
    E_field = E_0*(frac_gauss*np.exp(-v**2/2.) + (1-frac_gauss)/(1+v**2)) + E_1*np.exp(-view_angle_deg**2/2./width2**2)
    E_field *= 86.4/distance_km   # distance to tau decay point correction
...
def E_field_model_ANITA_VHF_87deg(view_angle_deg, log10_tau_energy, distance_km):
    # Lorentzian beam pattern based on Harm's results
    # Returns electric field peak in V/m
    
    # This is a more detailed fit to the lateral distribution function at 87 for a 200 - 1200 MHz band.
    E_0  = 9.89514986e-07    # V/m at ground level for 10^17 eV tau lepton.
    frac_gauss = 8.03620470e-01# fraction of Gaussian peak ( 1-frac_gauss is lorentzian)
    peak_view_angle = 1.54541580e+00 # degrees
    width = 6.43020301e-01      # degrees
    E_1 =8.66116786e-07   # V/m at ground level for 10^17 eV tau lepton.
    width2 = 1.30431074e+00          # degrees
    v = (view_angle_deg-peak_view_angle)/width 
       # The E_field model is a linear combination of a Gaussian and a Lorentzian centered at the peak view angle. A Gaussian centered at 0 deg is added to account for the central lobe. 
    E_field = E_0*(frac_gauss*np.exp(-v**2/2.) + (1-frac_gauss)/(1+v**2)) + E_1*np.exp(-view_angle_deg**2/2./width2**2)
    E_field *= 86.4/distance_km   # distance to tau decay point correction
...
def E_field_model_Mount_VHF_87deg(view_angle_deg, log10_tau_energy, distance_km):
    # Lorentzian beam pattern based on Harm's results
    # Returns electric field peak in V/m
    
    # This is a more detailed fit to the lateral distribution function at 87 for a 30 - 80 MHz band.
    E_0  = 9.89514986e-07    # V/m at ground level for 10^17 eV tau lepton.
    frac_gauss = 8.03620470e-01 # fraction of Gaussian peak ( 1-frac_gauss is lorentzian)
    peak_view_angle = 1.54541580e+00 # degrees
    width = 6.43020301e-01     # degrees
...
def E_field_model_Mount_UHF_87deg(view_angle_deg, log10_tau_energy, distance_km):
    # Lorentzian beam pattern based on Harm's results
    # Returns electric field peak in V/m
    
    # This is a more detailed fit to the lateral distribution function at 87 for a 30 - 80 MHz band.
    E_0  =1.22542427e-05    # V/m at ground level for 10^17 eV tau lepton.
    frac_gauss = 1.31616346e-01 # fraction of Gaussian peak ( 1-frac_gauss is lorentzian)
    peak_view_angle = 1.41098065e+00 # degrees
    width =1.31750506e-01     # degrees
    E_1 =1.85994903e-07  # V/m at ground level for 10^17 eV tau lepton.
    width2 = -8.85520551e-01# degrees
    v = (view_angle_deg-peak_view_angle)/width 
       # The E_field model is a linear combination of a Gaussian and a Lorentzian centered at the peak view angle. A Gaussian centered at 0 deg is added to account for the central lobe. 
    E_field = E_0*(frac_gauss*np.exp(-v**2/2.) + (1-frac_gauss)/(1+v**2)) + E_1*np.exp(-view_angle_deg**2/2./width2**2)
    E_field *= 86.4/distance_km   # distance to tau decay point correction
    ...
    for k in range(0,len(GEOM_theta_exit)):
        ...
        if( LUT_P_exit[idx] > 1.e-15):  # make sure the probability of this event is non-zero
            ...
            if(X0_dist < dist_to_detector[k]): # If the event is contained the range probability is 1. 
                ...
                Peak_E_field = E_field_model_ANITA_UHF_87deg(exit_view_angle[k]*180./np.pi, log10_tau_energy, dist_to_detector[k])
		Peak_Voltage = E_to_V_signal(Peak_E_field)
                if(Peak_Voltage > 5.*threshold_voltage):
